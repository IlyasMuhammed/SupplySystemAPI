 ...
 * - permRequired: role-permission gate (existing) — the user's JWT permissions.
 * - featureCode: org feature-toggle gate (MT-005) — GET /api/tenant/current's enabledFeatureCodes.
 ...
 *
 * Visibility (see filterItems): Super Admin sees everything except items NOT marked
 * superAdminOnly are also shown to them unfiltered; Organization Admin sees every item whose
 * feature is enabled, regardless of its own permission grants; everyone else needs both the
 * feature enabled AND at least one of the item's required permissions.
 */
type NavItem = MenuItem & { permRequired?: string[]; featureCode?: string; superAdminOnly?: boolean; items?: NavItem[] };
    ...
    private filterItems(items: NavItem[], ctx: { isSuperAdmin: boolean }): NavItem[] {
        ...
        for (const item of items) {
            ...
            if (item.superAdminOnly) {
                ...
                if (item.featureCode && !this.tenantService.hasFeature(item.featureCode)) continue;
